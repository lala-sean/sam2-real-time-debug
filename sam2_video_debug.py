...
# SAM2 model
sam2_checkpoint = "//mnt/iMVR/shuojue/code/segment-anything-2-real-time/checkpoints/sam2_hiera_small.pt"
model_cfg = "//mnt/iMVR/shuojue/code/segment-anything-2-real-time/sam2_configs/sam2_hiera_s.yaml"
predictor = build_sam2_camera_predictor(model_cfg, sam2_checkpoint, device=device, apply_postprocessing=False)
...
for i, frame_file in enumerate(frame_files):
    ...
    if i in [0]:
        ...
        # initialize the predictor with the first frame and prompt
        predictor.load_first_frame(frame_rgb)
        clicked_torch = torch.tensor(clicked, dtype=torch.float32).to(device)
        labels_torch = torch.tensor(labels, dtype=torch.int64).to(device)
        _, out_obj_ids, out_mask_logits = predictor.add_new_prompt(frame_idx = i, obj_id = 1, points = clicked_torch, labels= labels)
        ...
        for point, label in zip(clicked, labels):
            ...
            # write the image
            cv2.imwrite(f'display_video/initial_{i}.png', cv2.cvtColor(frame_copy, cv2.COLOR_RGB2BGR))
    

    # Run SAM2
    inference_start = time.time()
    out_obj_ids, out_mask_logits = predictor.track(frame_rgb)
    # sigmoid to process the mask logits
    out_mask = torch.sigmoid(out_mask_logits)>0.5
    output = out_mask.cpu().numpy()
    output = output[0]*1.0
    ...
    # out.write(output)
    # Save output to file
    cv2.imwrite(f'display_video/output_{i}.png', output)
